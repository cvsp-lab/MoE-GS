...
def training(dataset, emb, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    and STG), prepares their associated Gaussian and Scene 
    instances, and sets up optimizers for training expert-specific weights.

    Designed specifically for training on the **Technicolor Dataset**, 
    where the multi-expert integration framework is optimized for 
    dynamic and complex motion reconstruction scenarios.

    Unlike the N3V dataset, which requires two STG models (0-149 and 150-299 frames)
    due to its longer sequence length, the Technicolor dataset contains only about 
    50 frames per scene, and thus employs a **single STG model** for reconstruction.
    ...
    b_bg = torch.tensor([0, 0, 0], dtype=torch.float32, device="cuda")
    ...
    scene_s_fh = SScene(
        ...
        stg_path=dataset.stg_path, init_mixprob = args.init_mixprob
    )
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        render_pkg = render(
            viewpoint_cam, gaussians, gaussians_e, gaussians_s_fh, 
            pipe, bg, b_bg, near=dataset.near, 
        ...
        stacked_weight = F.softmax(
            torch.stack([weight, e_weight, s_weight], dim=0) / args.temperature, dim=0
        )
        weight, e_weight, s_weight = (
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, scene_e, scene_s_fh, pipe, bg, b_bg, dataset.near, dataset.far, emb, rbfbasefunction, args.save_path)
            if (iteration in saving_iterations):
                ...
                stg_fh_path = os.path.join(args.save_path, "STG")
                scene_s_fh.save(iteration, stg_fh_path)
            
            if iteration < opt.iterations:
                # Apply optimizer step and clear gradients
                for model in [
                    gaussians, gaussians_e, gaussians_s_fh,
                ]:
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, scene_e : Scene, scene_s_fh : Scene, pipe, bg, b_bg, near, far, emb, rbfbasefunction, save_path):
    if tb_writer:
    ...
    if iteration in testing_iterations:
        ...
        for idx, viewpoint in enumerate(tqdm(test_viewpoint_stack)):
            ...
            render_pkg = render(
                ...
                scene_s_fh.gaussians, pipe, bg, b_bg, near=near, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[2000, 5000, 6000, 7000, 8000, 9000, 10000, 15000, 20000, 7000_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[2000, 5000])
