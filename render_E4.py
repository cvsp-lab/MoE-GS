...
def render_set(save_path, name, iteration, scene, gaussians, gaussians_e, gaussians_s_fh, gaussians_s_sh, gaussians_h, pipeline, background, fd_background, inverval=1, near=0.2, far=100.0, save_img=False, embparam=None, E_iter = None):
    ...
    i_images, e_images, s_images, h_images = [], [], [], []
    ...
    while(len(viewpoint_stack)):
        ...
        if idx % inverval == 0:
            render_pkg = render(
                cam, gaussians, gaussians_e, 
                gaussians_s_fh, gaussians_s_sh, gaussians_h, 
                pipeline, background, fd_background, near=near, 
                far=far,iter=E_iter, 
...
def render_sets(dataset : ModelParams, embparam, hyperparam, iteration : int, opt : OptimizationParams, pipeline : PipelineParams, skip_train : bool, train_inverval : int, skip_test : bool, save_img : bool, save_path : str,
                gaussian_dim : int, time_duration : float, num_pts : int, num_pts_ratio : float, rot_4d : bool, force_sh_3d : bool, batch_size : int, re_dataset):
    """
    ...
    with torch.no_grad():
        ...
        # --- STG (first half / second half) ---
        stg_fh_path = os.path.join(save_path, 'STG', '0to149', 'point_cloud', 'iteration_' + str(iteration))
        stg_sh_path = os.path.join(save_path, 'STG', '150to299', 'point_cloud', 'iteration_' + str(iteration))
        ...
        gaussians_s_sh = SGaussianModel(dataset.sh_degree, args.rgbfunction)
        scene_s_fh = SScene(dataset, gaussians_s_sh, load_iteration=iteration, shuffle=False, render=True, save_path=save_path, stg_path=stg_sh_path)
        ...
        fd_background = torch.tensor(fd_bg_color, dtype=torch.float32, device="cuda")
        
        if not skip_train:
            print(f"[INFO] Rendering train viewpoints ...")
            render_set(save_path, "train", scene.loaded_iter, scene, gaussians, gaussians_e, gaussians_s_fh, gaussians_s_sh, gaussians_h, pipeline, background, fd_background, near=dataset.near, far=dataset.far, save_img=save_img, embparam=embparam, E_iter = scene_e.pretrained_iter)

        if not skip_test:
            print(f"[INFO] Rendering test viewpoints ...")
            render_set(save_path, "test", scene.loaded_iter, scene, gaussians, gaussians_e, gaussians_s_fh, gaussians_s_sh, gaussians_h, pipeline, background, fd_background, near=dataset.near, far=dataset.far, save_img=save_img, embparam=embparam, E_iter = scene_e.pretrained_iter)
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), emb.extract(args), hp.extract(args), args.iteration, opt.extract(args), pipeline.extract(args), args.skip_train, args.train_inverval, args.skip_test, args.save_img, args.save_path,
                args.gaussian_dim, args.time_duration, args.num_pts, args.num_pts_ratio, args.rot_4d, args.force_sh_3d, args.batch_size, re_ModelParams)