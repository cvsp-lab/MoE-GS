...
from gaussian_renderer import render_E4 as render
...
def training(dataset, emb, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args, re_dataset):
    """
    Main training routine for the MoE-GS framework.

    This function initializes and loads multiple expert models (Ex4DGS, E-D3DGS, 
    4DGaussians, and STG), prepares their associated Gaussian and Scene 
    instances, and sets up optimizers for training expert-specific weights.

    In Expert 4 setting, we use Ex4DGS & E-D3DGS & 4DGaussians(CVPR) & STG
    For STG data issues(max 150 frame training on A100), we use two STG models each reconstruct 0 to 149 and 150 to 299
    When short time reconsctruction, it can simiply deleted for second half code
    ...
    """
    In the 4-expert configuration, we use:
        - Ex4DGS
        - E-D3DGS
        - 4DGaussians (CVPR)
    ...
    Note:
        For STG, due to GPU memory limitations on A100 (max 150 frames per model),
        two separate models are used:
            - STG_0to149: reconstructs frames 0-149
            - STG_150to299: reconstructs frames 150-299
        For shorter sequences, the second model can be omitted safely.
    """
    ...
    fd_bg_color = [1, 1, 1] if re_dataset.white_background else [0, 0, 0]
    fd_bg = torch.tensor(fd_bg_color, dtype=torch.float32, device="cuda")
    ...
    # Load Two STG Models (0–149, 150–299)
    rbfbasefunction = trbfunction
    stg_fh_path = os.path.join(dataset.stg_path, dataset.stg_path.split('/')[-1]+'_0to149')
    stg_sh_path = os.path.join(dataset.stg_path, dataset.stg_path.split('/')[-1]+'_150to299')
    
    gaussians_s_fh = SGaussianModel(dataset.sh_degree, rgbfunction=args.rgbfunction)
    gaussians_s_fh.trbfslinit = -1*opt.trbfslinit # 
    gaussians_s_fh.preprocesspoints = opt.preprocesspoints 
    gaussians_s_fh.addsphpointsscale = opt.addsphpointsscale 
    gaussians_s_fh.raystart = opt.raystart
    scene_s_fh = SScene(
        dataset, gaussians_s_fh, args.load_iteration, duration=args.duration, loader=dataset.loader, 
        cameras_extent=cameras_extent, save_path=args.save_path, 
        stg_path=stg_fh_path, init_mixprob = args.init_mixprob
    )

    gaussians_s_sh = SGaussianModel(dataset.sh_degree, rgbfunction=args.rgbfunction)
    gaussians_s_sh.trbfslinit = -1*opt.trbfslinit # 
    gaussians_s_sh.preprocesspoints = opt.preprocesspoints 
    gaussians_s_sh.addsphpointsscale = opt.addsphpointsscale 
    gaussians_s_sh.raystart = opt.raystart
    scene_s_sh = SScene(
        stg_sh_path, gaussians_s_sh, args.load_iteration, duration=args.duration, loader=dataset.loader, 
        cameras_extent=cameras_extent, save_path=args.save_path, 
        stg_path=stg_sh_path, init_mixprob = args.init_mixprob
    )
    ...
    gaussians_s_fh.training_setup(opt)
    gaussians_s_sh.training_setup(opt)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        gaussians_s_fh.update_learning_rate(iteration)
        gaussians_s_sh.update_learning_rate(iteration)
        ...
        render_pkg = render(
            viewpoint_cam, gaussians, gaussians_e, 
            gaussians_s_fh, gaussians_s_sh, gaussians_h, 
            pipe, bg, fd_bg, near=dataset.near, 
        ...
        image, e_image, s_image, h_image, = (
            render_pkg["render"], render_pkg["e_render"], 
            render_pkg["s_render"], render_pkg["h_render"],
        )
        weight, e_weight, s_weight, h_weight = (
            render_pkg["weight"], render_pkg["e_weight"], 
            render_pkg["s_weight"], render_pkg["h_weight"],
        )
        ...
        stacked_weight = F.softmax(
            torch.stack([weight, e_weight, s_weight, h_weight], dim=0) / args.temperature, dim=0
        )
        weight, e_weight, s_weight, h_weight = (
            stacked_weight[0].unsqueeze(0), 
            stacked_weight[1].unsqueeze(0), 
            stacked_weight[2].unsqueeze(0), 
            stacked_weight[3].unsqueeze(0), 
        )
        moe_image = (
            (image * weight)
            + (e_image * e_weight)
        ...
        with torch.no_grad():
            ...
            # Log and save
            test_psnr = training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, scene_e, scene_s_fh, scene_s_sh, scene_h, pipe, bg, fd_bg, dataset.near, dataset.far, emb, rbfbasefunction, args.save_path)
            if (iteration in saving_iterations):
                ...
                scene_s_fh.save(iteration, stg_fh_path)
                scene_s_sh.save(iteration, stg_sh_path)
            
            if iteration < opt.iterations:
                # Apply optimizer step and clear gradients
                for model in [
                    gaussians, gaussians_e,
                    gaussians_s_fh, gaussians_s_sh, gaussians_h
                ]:
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, scene_e : Scene, scene_s_fh : Scene, scene_s_sh : Scene, scene_h : Scene, pipe, bg, fd_bg, near, far, emb, rbfbasefunction, save_path):
    if tb_writer:
   ...
   # Report test and samples of training set
    if iteration in testing_iterations:
        ...
        weights, e_weights, s_weights, h_weights = [], [], [], []

        images, e_images, s_images, h_images = [], [], [], []

        for idx, viewpoint in enumerate(tqdm(test_viewpoint_stack)):
            gt = next(test_images).cuda()

            render_pkg = render(
                viewpoint, scene.gaussians, scene_e.gaussians, 
                scene_s_fh.gaussians, scene_s_sh.gaussians, scene_h.gaussians, 
                pipe, bg, fd_bg, near=near, 
                far=far,iter=scene_e.pretrained_iter, 
                num_down_emb_c=emb.min_embeddings, 
                num_down_emb_f=emb.min_embeddings, 
                cam_no=None, basicfunction=rbfbasefunction
            )
            ...
            # Pixel-wise MoE fusion
            stacked_weight = F.softmax(
                torch.stack([weight, e_weight, s_weight, h_weight], dim=0), dim=0
            )
            weight, e_weight, s_weight, h_weight = (
                stacked_weight[0].unsqueeze(0), 
                stacked_weight[1].unsqueeze(0), 
        ...
        psnr_test_iter = torch.tensor(moe_psnrs).mean().item()
        torch.cuda.empty_cache()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[500, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 15000, 20000, 7000_000])
    ...
    # Load 4DGS(iclr) Config
    reparam_cfg_path = f"thirdparty/reparmetrize/configs/dynerf/{os.path.basename(args.source_path)}.yaml"
    if not os.path.exists(reparam_cfg_path):
        raise FileNotFoundError(f"Missing 4DGS config: {reparam_cfg_path}")
    reparam_cfg = mmcv.Config.fromfile(reparam_cfg_path)
    re_ModelParams = reparam_cfg.ModelParams
    re_PipelineParams = reparam_cfg.PipelineParams

    torch.autograd.set_detect_anomaly(args.detect_anomaly)
    training(lp.extract(args), ep.extract(args), hp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args, re_ModelParams)
