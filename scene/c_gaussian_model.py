...
class CGaussianModel:
    ...
    def __init__(self, sh_degree : int, duration : int, interval : int, time_pad : int = 1, interp_type='linear', rot_interp_type='slerp', time_pad_type=0, var_pad=3, kernel_size=0.1, rgbfunction="rgbv1", **kwargs):
        ...
        self.weight_gradient_accum = torch.empty(0).cuda()
        ...
        self.rgbdecoder = getcolormodel(rgbfunction)
        ...
        self.motion_weight_gradient_accum = torch.empty(0).cuda()
        ...
        if interp_type == "linear":
            ...
            def pos_interpolation(y, t_idx, delta_t):
            ...
            self.motion_degree = 1 # points, diff
            def pos_interpolation(y, y_d, t_idx, delta_t):
                if type(t_idx) == torch.Tensor:
                    y1 = torch.gather(y, 1, t_idx.repeat(1, 1, 3 * self.motion_degree))
                    y2 = torch.gather(y, 1, t_idx.repeat(1, 1, 3 * self.motion_degree) + 1)
        ...
        self.interpolator = pos_interpolation
    ...
    def get_motion_mask(self, deltat, deltat_motion):
        return torch.cat((
            torch.cat((self._spacetime_mask, deltat * self._features_t), dim=1),
            torch.cat((self._spacetime_mask_motion, deltat_motion * self._features_motion_t), dim=1)), dim=0)
    
    def get_mask_td(self, deltat, deltat_motion):
        return torch.cat((
                torch.cat((self._features_d, deltat * self._features_t), dim=1),
                torch.cat((self._features_motion_d, deltat_motion * self._features_motion_t), dim=1)), dim=0)
    ...
    def get_motion_opacity_with_weights(self, t, training=False):
        t = (t + self.time_shift) / self.interval
        motion_opaicty = self.get_motion_opacity.detach()
        motion_mask  = self.opacity_activation(self._spacetime_mask_motion)
        motion_opacity = motion_opaicty*motion_mask.unsqueeze(1)
        return (time_bigaussian(self._opacity_duration_center.detach(), self._opacity_duration_var.detach(), t, training=training, var_min=self.var_pad/self.interval)
                            * motion_opacity)
    ...
    def get_opacity_at_t_with_weight(self, t, mode=0, training=False):
        static_opacity = self.get_opacity.detach()
        # sigmoid fuction
        static_mask  = self.opacity_activation(self._spacetime_mask)
        static_opacity = static_opacity*static_mask.unsqueeze(1)
        
        dynamic_opacity = self.get_motion_opacity_with_weights(t, training=training)

        return torch.cat([static_opacity, 
                          dynamic_opacity], dim=0).contiguous()
    ...
    def training_setup(self, training_args):
        ...
        self.weight_gradient_accum = torch.zeros((self._xyz.shape[0], 1), device="cuda")
        ...
        self.motion_weight_gradient_accum = torch.zeros((self._xyz_motion.shape[0], 1), device="cuda")
        ...
        l = [
            ...
            {'params': [self._spacetime_mask], 'lr': training_args.featuret_lr, "name": "f_m"},
            {'params': [self._spacetime_mask_motion], 'lr': training_args.featuret_lr, "name": "motion_f_m"},
            {'params': [self._features_t], 'lr': training_args.feature_lr, "name": "f_t"},
            {'params': [self._features_motion_t], 'lr': training_args.feature_lr, "name": "motion_f_t"},
            # {'params': [self._features_d], 'lr': training_args.feature_lr, "name": "f_d"},
            # {'params': [self._features_motion_d], 'lr': training_args.feature_lr, "name": "f_d"},
        ...
        self.weight_scheduler_args = get_expon_lr_func(lr_init=training_args.featuret_lr,
                                                    lr_final=training_args.featuret_lr*0.01,
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                ...
                param_group['lr'] = lr
                # return lr
            elif param_group["name"] == "f_m":
                lr = self.weight_scheduler_args(iteration)
                param_group['lr'] = lr
            # elif param_group["name"] == "f_t":
    ...
    def construct_list_of_static_attributes(self):
        ...
        for i in range(self._spacetime_mask.shape[1]):
            l.append('w_b_{}'.format(i))     
        return l
    ...
    def construct_list_of_dynamic_attributes(self):
        ...
        for i in range(self._features_motion_t.shape[1]):
            l2.append('motion_f_t_{}'.format(i))
        for i in range(self._spacetime_mask_motion.shape[1]):
            l2.append('motion_w_b_{}'.format(i))     
    ...
    def save_ply(self, path):
        ...
        spacetime_mask = self._spacetime_mask.detach().cpu().numpy()
        ...
        static_attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, xyz_disp, f_t, spacetime_mask), axis=1)
        ...
        f_t_motion =  self._features_motion_t.detach().cpu().numpy()
        spacetime_mask_motion = self._spacetime_mask_motion.detach().cpu().numpy()
        ...
        dynamic_attributes = np.concatenate((xyz_motion, f_dc_motion, f_rest_motion, scaling_motion, opacity_motion, opacity_duration_center, opacity_duration_var, rotation_motion, f_t_motion, spacetime_mask_motion), axis=1)
    ...
    def load_ply(self, path, spatial_lr_scale:float, init_mixprob:float, search=False):
        self.spatial_lr_scale = spatial_lr_scale
        ...
        # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        motion_features_extra = motion_features_extra.reshape((motion_features_extra.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1))  # pylint: disable=unsubscriptable-object
        ...
        if search:
            init_weight = init_mixprob * torch.ones((self._xyz.shape[0],1), dtype=torch.float, device="cuda")
            spacetime_mask = torch.cat((init_weight, init_weight), dim=1)

            init_weight_motion = init_mixprob * torch.ones((self._xyz_motion.shape[0], 1), dtype=torch.float, device="cuda")
            spacetime_mask_motion = torch.cat((init_weight_motion, init_weight_motion), dim=1)


            self._spacetime_mask = nn.Parameter(spacetime_mask.requires_grad_(True))
            self._spacetime_mask_motion = nn.Parameter(spacetime_mask_motion.requires_grad_(True))

            fomega = torch.zeros((self._xyz.shape[0], 1), dtype=torch.float, device="cuda")
            self._features_t =  nn.Parameter(fomega.contiguous().requires_grad_(True))
            fomega_motion = torch.zeros((self._xyz_motion.shape[0], 1), dtype=torch.float, device="cuda")
            self._features_motion_t =  nn.Parameter(fomega_motion.contiguous().requires_grad_(True))

        else:
            weight_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("w_b_")]
            spacetime_mask = np.zeros((xyz.shape[0], len(weight_names)))
            for idx, attr_name in enumerate(weight_names):
                spacetime_mask[:, idx] = np.asarray(plydata.elements[0][attr_name])
            self._spacetime_mask = nn.Parameter(torch.tensor(spacetime_mask, dtype=torch.float, device="cuda").requires_grad_(True))
            
            ft_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("f_t_")]
            ...
            motion_weight_names = [p.name for p in plydata_dynamic.elements[0].properties if p.name.startswith("motion_w_b_")]
            spacetime_mask = np.zeros((num_points, len(weight_names)))
            for idx, attr_name in enumerate(motion_weight_names):
                spacetime_mask[:, idx] = np.asarray(plydata_dynamic.elements[0][attr_name])
            self._spacetime_mask_motion = nn.Parameter(torch.tensor(spacetime_mask, dtype=torch.float, device="cuda").requires_grad_(True))

            ft_names = [p.name for p in plydata_dynamic.elements[0].properties if p.name.startswith("motion_f_t_")]
            ftomegas = np.zeros((num_points, len(ft_names)))
            for idx, attr_name in enumerate(ft_names):
                ftomegas[:, idx] = np.asarray(plydata_dynamic.elements[0][attr_name])
            self._features_motion_t = nn.Parameter(torch.tensor(ftomegas, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def prune_points(self, static_mask, dynamic_mask, reset_mode=False):
        ...
        self._spacetime_mask = self._spacetime_mask[vaild_static_mask]
        self._features_t = self._features_t[vaild_static_mask]
        ...
        if reset_mode:
            ...
            self.weight_gradient_accum = self.weight_gradient_accum[vaild_static_mask]
        ...
        if reset_mode:
            self.motion_xyz_gradient_accum = torch.zeros((self._xyz_motion.shape[0], 1), device="cuda")
            self.motion_denom = torch.zeros((self._xyz_motion.shape[0], 1), device="cuda")
            self.motion_xyz_error_min = torch.zeros((self._xyz_motion.shape[0], 1), device="cuda")
            self.motion_xyz_error_mean = torch.zeros((self._xyz_motion.shape[0], 1), device="cuda")
            self.motion_xyz_error_min_timestamp =torch.zeros((self._xyz_motion.shape[0], 1), device="cuda")
            self.motion_xyz_ssim_error_accum = torch.zeros((self._xyz_motion.shape[0], 1), device="cuda")
            self.motion_error_denom = torch.zeros((self._xyz_motion.shape[0], 1), device="cuda")
            self.motion_weight_gradient_accum = torch.zeros((self._xyz_motion.shape[0], 1), device="cuda")
            ...
            self.motion_weight_gradient_accum = self.motion_weight_gradient_accum[vaild_dynamic_mask]
            # self.motion_max_radii2D = self.motion_max_radii2D[vaild_dynamic_mask]
            # self.motion_min_radii2D = self.motion_min_radii2D[vaild_dynamic_mask]

        self._spacetime_mask_motion = self._spacetime_mask_motion[vaild_dynamic_mask]
        self._features_motion_t = self._features_motion_t[vaild_dynamic_mask]
        
        
    def weight_pruning(self,static_mask, dynamic_mask):
        vaild_static_mask = ~static_mask
        vaild_dynamic_mask = ~dynamic_mask


        self._xyz = self._xyz[vaild_static_mask]
        self._features_dc = self._features_dc[vaild_static_mask]
        self._features_rest = self._features_rest[vaild_static_mask]
        self._opacity = self._opacity[vaild_static_mask]
        self._scaling = self._scaling[vaild_static_mask]
        self._rotation = self._rotation[vaild_static_mask]
        self._xyz_disp = self._xyz_disp[vaild_static_mask]
        self._spacetime_mask = self._spacetime_mask[vaild_static_mask]
        self._features_t = self._features_t[vaild_static_mask]

        # dynamic points
        self._xyz_motion = self._xyz_motion[vaild_dynamic_mask]
        self._features_dc_motion = self._features_dc_motion[vaild_dynamic_mask]
        self._features_rest_motion = self._features_rest_motion[vaild_dynamic_mask]
        self._scaling_motion = self._scaling_motion[vaild_dynamic_mask]
        self._opacity_motion = self._opacity_motion[vaild_dynamic_mask]
        self._opacity_duration_center = self._opacity_duration_center[vaild_dynamic_mask] 
        self._opacity_duration_var = self._opacity_duration_var[vaild_dynamic_mask]
        self._rotation_motion = self._rotation_motion[vaild_dynamic_mask]
        self._spacetime_mask_motion = self._spacetime_mask_motion[vaild_dynamic_mask]
        self._features_motion_t = self._features_motion_t[vaild_dynamic_mask]
    ...
    def add_weight_grad(self, static_grad, dynamic_grad, static_update_filter, dynamic_update_filter):
        # self.weight_gradient_accum[static_update_filter] += static_grad[static_update_filter]
        self.weight_gradient_accum[static_update_filter] += torch.norm(static_grad[static_update_filter], dim=-1, keepdim=True)     # [n_gaussians, 2] for spacetime_mask
        self.denom[static_update_filter] += 1

        if self.motion_weight_gradient_accum.shape[0] == 0:
            return

        # self.motion_weight_gradient_accum[dynamic_update_filter] += dynamic_grad[dynamic_update_filter]
        self.motion_weight_gradient_accum[dynamic_update_filter] += torch.norm(dynamic_grad[dynamic_update_filter], dim=-1, keepdim=True)
        self.motion_denom[dynamic_update_filter] += 1
